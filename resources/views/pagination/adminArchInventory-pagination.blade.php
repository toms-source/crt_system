<section>
    ...
    <div class="xs:mb-6 md:mb-6 mt-6 flex justify-center">
        <nav class="flex space-x-2" aria-label="Pagination">
            @php
            $totalPages = $adminArchiveInventory->lastPage();
            $currentPage = $adminArchiveInventory->currentPage();
            $maxPagesToShow = 5;
            ...
            @if ($startPage > 1)
                <a href="{{ $adminArchiveInventory->url(1) }}"
            ...
            @for ($page = $startPage; $page <= $endPage; $page++)
                ...
                @else
                    <a href="{{ $adminArchiveInventory->url($page) }}"
                        class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 dark:bg-stone-700 dark:text-gray-200 dark:hover:bg-stone-600">
                        {{ $page }}
                    </a>
                @endif
            ...
            @if ($endPage < $totalPages)
                @if ($endPage < $totalPages - 1)
                    <span class="px-4 py-2 text-gray-400 dark:text-gray-500">...</span>
                @endif
                <a href="{{ $adminArchiveInventory->url($totalPages) }}"
                    class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 dark:bg-stone-700 dark:text-gray-200 dark:hover:bg-stone-600">
                    {{ $totalPages }}
                </a>
            @endif
            ...
            <!-- Next Button -->
            @if ($adminArchiveInventory->hasMorePages())
                <a href="{{ $adminArchiveInventory->nextPageUrl() }}"
                    class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-stone-700 dark:bg-stone-700 dark:hover:bg-stone-950">
                    Next
                </a>
            @else
